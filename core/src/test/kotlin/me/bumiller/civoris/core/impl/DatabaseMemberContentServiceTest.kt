...
import me.bumiller.civoris.database.repository.MemberRoleRepository
import me.bumiller.civoris.database.repository.UserRepository
import me.bumiller.civoris.database.table.LawBook
import me.bumiller.civoris.model.MemberRole
import me.bumiller.civoris.model.User
import me.bumiller.civoris.test.util.lawBookEntity
import me.bumiller.civoris.test.util.userEntities
import me.bumiller.civoris.test.util.userEntity
...
class DatabaseMemberContentServiceTest {
    ...
    fun `addMemberToBook only calls update with correctly updated member field`() = runTest {
        ...
        assertArrayEquals(
            arrayOf(1L, 2L, 3L, 4L),
            bookSlot.captured.members.map(me.bumiller.civoris.database.table.User.Model::id).sorted().toTypedArray()
        )
    }
